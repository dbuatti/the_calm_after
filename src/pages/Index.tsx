...
import { Wind, Shield, ArrowRight, Sparkles, Heart, AlertCircle, Star, Clock, Play, LayoutGrid } from 'lucide-react';
      ...
      >
        <header className="text-center space-y-8 md:space-y-12">
          ...
          <motion.div variants={itemVariants} className="flex flex-wrap justify-center gap-4 md:gap-6 px-4">
            ...
            <Button 
              onClick={() => navigate('/tools?panic=true')} 
              className="bg-rose-500 text-white hover:bg-rose-600 rounded-full px-10 md:px-12 h-14 md:h-16 font-black uppercase tracking-widest text-[10px] md:text-xs shadow-2xl shadow-rose-500/20 transition-all hover:scale-105 active:scale-95"
            >
              <AlertCircle className="mr-2 h-4 w-4 animate-pulse" /> Panic Mode
            </Button>
            <Button 
            ...
            >
              <LayoutGrid className="mr-2 h-4 w-4" /> Full Toolbox
            </Button>
          </motion.div>
        </header>
        ...
        {(favorites.length > 0 || lastUsed) && (
          <motion.section variants={itemVariants} className="space-y-6 md:space-y-8 px-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
              {lastUsed && (
                <Card 
                  className="bg-white/[0.02] border-white/10 hover:bg-white/5 transition-all cursor-pointer group rounded-[32px] active:scale-95"
                  onClick={() => navigate('/tools')}
                >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 px-4">
          <motion.div variants={itemVariants}>
            <Card 
              className="bg-white/[0.02] backdrop-blur-3xl border-white/10 text-white hover:bg-white/[0.05] transition-all duration-700 cursor-pointer group h-full overflow-hidden relative rounded-[64px] active:scale-[0.98]" 
              onClick={() => navigate('/session')}
            >
              ...
              <CardContent className="p-10 md:p-16 flex flex-col justify-between h-full space-y-12 md:space-y-16 relative z-10">
                <div className="space-y-8 md:space-y-10">
                  <div className="w-20 h-20 md:w-24 md:h-24 bg-sky-500/10 rounded-[32px] flex items-center justify-center text-sky-400 group-hover:scale-110 group-hover:rotate-3 transition-all duration-700">
                ...
                <Button variant="ghost" className="w-full justify-between group-hover:bg-white/5 h-16 md:h-20 text-xl md:text-2xl font-bold rounded-[32px] px-8 md:px-10">
          ...
          <motion.div variants={itemVariants}>
            ...
            >
              ...
              <CardContent className="p-10 md:p-16 flex flex-col justify-between h-full space-y-12 md:space-y-16 relative z-10">
                <div className="space-y-8 md:space-y-10">
                  <div className="w-20 h-20 md:w-24 md:h-24 bg-purple-500/10 rounded-[32px] flex items-center justify-center text-purple-400 group-hover:scale-110 group-hover:-rotate-3 transition-all duration-700">
